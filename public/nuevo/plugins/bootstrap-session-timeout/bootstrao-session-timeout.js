    ...
    a.sessionTimeout = function (b) {
        ...
        function f(b, c) {
            clearTimeout(j.timer),
                "dialog" === b && c ? (j.timeLeft = Math.floor((i.redirAfter - i.warnAfter) / 1e3)) : "session" === b && c && (j.timeLeft = Math.floor(i.redirAfter / 1e3)),
        ...
        if ("function" != typeof i.onWarn) {
            var k = i.countdownMessage ? "<p>" + i.countdownMessage.replace(/{timer}/g, '<span class="countdown-holder"></span>') + "</p>" : "",
                l = i.countdownBar
                    ...
                    : "";
            a("body").append(
                '<div class="modal fade" id="session-timeout-dialog">               <div class="modal-dialog">                 <div class="modal-content">                   <div class="modal-header">                                          <h6 class="modal-title mb-0">' +
                    i.title +
                    '</h6>          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>                  </div>                   <div class="modal-body">                     <p>' +
                    i.message +
                    "</p>                     " +
                    k +
                    "                     " +
                    ...
                    i.logoutButton +
                    '</button>                     <button id="session-timeout-dialog-keepalive" type="button" class="btn btn-primary btn-sm" data-bs-dismiss="modal">' +
                    i.keepAliveButton +
            ...
            a(document).on("keyup mouseup mousemove touchend touchmove", function (b) {
                ...
                d(),
                    a("#session-timeout-dialog").length > 0 &&
                        ...
                        (a("#session-timeout-dialog").modal("hide"), a("body").removeClass("modal-open"), a("div.modal-backdrop").remove());
            });
        }
        var n = !1;
        d();
